...
#define DEVICE_SIFC_QUIZKING  RETRO_DEVICE_SUBCLASS(RETRO_DEVICE_JOYPAD, 8)
#define DEVICE_SIFC_4PLAYER   RETRO_DEVICE_SUBCLASS(RETRO_DEVICE_JOYPAD, 9)
...
unsigned char turbo_button_toggle[MAX_CONTROLLERS][TURBO_BUTTONS] = { {0} };
...
static void set_input(unsigned port) {
	...
	if (((iNESCart.InputTypes == 0x02) || (nes_input.type[4] != DEVICE_SIFC_4PLAYER))
	&& ((nes_input.type[2] == DEVICE_SI_GAMEPAD) || (nes_input.type[3] == DEVICE_SI_GAMEPAD))) {
	...
	if (nes_input.type[4] == DEVICE_SIFC_4PLAYER)
		FCEUI_DisableFourScore(1);
}
...
void update_input_descriptors(void) {
	...
	{
		if (nes_input.type[port] == DEVICE_SI_GAMEPAD || nes_input.type[port] == RETRO_DEVICE_JOYPAD || nes_input.type[port] == DEVICE_SI_SNESGAMEPAD)
		{
			...
			if (GameInfo->type == GIT_VSUNI) {
				addDesc(&desc[i++], port, RETRO_DEVICE_ID_JOYPAD_L2,     "(VS) Insert Coin (slot 1)" );
				addDesc(&desc[i++], port, RETRO_DEVICE_ID_JOYPAD_R2,     "(VS) Insert Coin (slot 2)" );
			}

			if (iNESCart.mapper == 124) {
				addDesc(&desc[i++], 0, RETRO_DEVICE_ID_JOYPAD_L2,     "Insert Coin" );
				addDesc(&desc[i++], 0, RETRO_DEVICE_ID_JOYPAD_R2,     "Insert Coin" );
			}
		}
		...
		else if (nes_input.type[port] == DEVICE_SIFC_QUIZKING)
		{
			addDesc(&desc[i++], port, RETRO_DEVICE_ID_JOYPAD_B, "B1");
			addDesc(&desc[i++], port, RETRO_DEVICE_ID_JOYPAD_A, "B2");
			addDesc(&desc[i++], port, RETRO_DEVICE_ID_JOYPAD_Y, "B3");
...
void set_controller_port_device(unsigned port, unsigned device) {
	if (device == DEVICE_AUTO) {
		if (port <= 1) {
			...
			if (nes_input.enable_4player || nes_input.type[4] == DEVICE_SIFC_4PLAYER || iNESCart.InputTypes == 0x02) {
				nes_input.type[port] = DEVICE_SI_GAMEPAD;
			}
		} else {
			/* famicom expansion port */
			...
			case SIFC_FTRAINERB:
				...
				break;
			case SIFC_QUIZKING:
				nes_input.type[4] = DEVICE_SIFC_QUIZKING;
				break;
			default:
