...
static void SetType(int type)
{
	...
	OPLL_setChipType(chip, t);
	OPLL_resetPatch(chip, t);
}

static void VRC7SC(void) {
	if (chip) {
		if (!FSettings.VRC7ToneType)
			SetType(opll_type);
		else
			SetType(FSettings.VRC7ToneType);
...
void VRC7Sound_AddStateInfo(void) {
	...
	AddExState(&chip->slot[13], sizeof(chip->slot[13]), 0, "SL13");
	AddExState(&chip->slot[14], sizeof(chip->slot[14]), 0, "SL14");
	AddExState(&chip->slot[15], sizeof(chip->slot[15]), 0, "SL15");
