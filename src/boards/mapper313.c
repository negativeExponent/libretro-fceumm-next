...
static void M313CW(uint32 A, uint8 V) {
	...
	case 2: bank = (mmc3.expregs[0] << 8) | (V & 0xFF); break;
	case 3: bank = (mmc3.expregs[0] << 8) | (V & 0xFF); break;
	case 4: bank = (mmc3.expregs[0] << 7) | (V & 0x7F); break;
	}
	setchr1(A, bank);
}
...
static void M313PW(uint32 A, uint8 V) {
	...
	case 2: bank = (mmc3.expregs[0] << 4) | (V & 0x0F); break;
	case 3: bank = (mmc3.expregs[0] << 5) | (V & 0x1F); break;
	case 4:
		if (mmc3.expregs[0] == 0)
			bank = (mmc3.expregs[0] << 5) | (V & 0x1F);
		else
			bank = (mmc3.expregs[0] << 4) | (V & 0x0F);
		break;
	}
	setprg8(A, bank);
}
