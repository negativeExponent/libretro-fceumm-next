...
static void CheckHInfo(CartInfo *info, uint64 partialmd5) {
	...
	do {
		if (moo[x].crc32 == info->CRC32) {
			...
			if (moo[x].mirror >= 0) {
				if (info->mirror != moo[x].mirror) {
					/* TODO: Verify this condition */
					if (info->mirror != (moo[x].mirror & ~MI_4))
						if ((moo[x].mirror & ~MI_4) <= 2) /* Don't complain if one-screen mirroring
			...
			if (moo[x].prgram >= 0) {
				int prgram = (moo[x].prgram & 0x0F) ? (64 << ((moo[x].prgram >> 0) & 0xF)) : 0;
				int prgsaveram = (moo[x].prgram & 0xF0) ? (64 << ((moo[x].prgram >> 4) & 0xF)) : 0;
			...
			if (moo[x].chrram >= 0) {
				int chrram = (moo[x].chrram & 0x0F) ? (64 << ((moo[x].chrram >> 0) & 0xF)) : 0;
				int chrsaveram = (moo[x].chrram & 0xF0) ? (64 << ((moo[x].chrram >> 4) & 0xF)) : 0;
	...
	if (tofix) {
		...
		if (tofix & 32) {
			...
			if (PRGRAM || CHRRAM) {
				...
				FCEU_printf(" [DB] CHR RAM: %d KB.", CHRRAM / 1024);
			}
		}
	}
}
...
int iNESLoad(const char *name, FCEUFILE *fp) {
	...
	if (iNESCart.mirror == MI_4) {
		...
		SetupCartMirroring(iNESCart.mirror, TRUE, ExtraNTARAM);
	} else {
		SetupCartMirroring(iNESCart.mirror, (iNESCart.mirror & 2) ? TRUE : FALSE, NULL);
	}
