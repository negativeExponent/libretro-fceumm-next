...
void RDoPCM(void) {
	...
	for (V = ChannelBC[4]; V < SOUNDTS; V++) {
		...
		WaveHi[V] += GetSndOutput(APU_DPCM, RawDALatch << TRINPCM_SHIFT) & (~0xFFFF);
	}
...
static INLINE void RDoSQ(int x) {
	...
	amp = GetSndOutput(APU_SQUARE1 + x, amp);
	amp <<= SQ_SHIFT;
...
static void RDoSQLQ(void) {
	...
	for (x = 0; x < 2; x++) {
		...
		amp[x] = GetSndOutput(APU_SQUARE1 + x, amp[x]);
...
static INLINE int32 GetTriSample(void) {
	...
	sample = (sample * 3) << TRINPCM_SHIFT;
	return GetSndOutput(APU_TRIANGLE, sample);
}
...
static void RDoTriangleNoisePCMLQ(void) {
    ...
    pcmout = GetSndOutput(APU_DPCM, RawDALatch);
    amptab[0] = GetSndOutput(APU_NOISE, amptab[0]);
 ...
 rea:
				...
				lq_tcout = lq_tcout * 3;
                lq_tcout = GetSndOutput(APU_TRIANGLE, lq_tcout);
				totalout = wlookup2[lq_tcout + noiseout + pcmout];
			}
...
int32 GetSndOutput(int channel, int32 in) {
	if ((channel > APU_MASTER) && (channel < APU_LAST)) {
		int mod = (int)FCEUI_GetSoundVolume(channel);
