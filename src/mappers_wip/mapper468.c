...
static struct {
	uint8 reg[4];        /* Supervisor registers */
	uint8 Custom_reg[4]; /* Registers for custom mappers */
	uint8 eeprom[16], eep_clock, state, command, output; /* Serial EEPROM */
} m468;
...
static void setPins(uint8 select, uint8 newClock, uint8 newData) { /* Serial EEPROM */
	if (select) {
		...
		if (m468.state < 8) {
			...
			uint32 mask = 1 << (15 - m468.state);
			uint32 address = m468.command & 0x0F;
...
static INLINE uint16 Mapper_GetPRGBase(void) {
...
static void Sync_AxROM(void) {
	uint16 prgMask = ((m468.reg[0] & 0x20) ? 0x0F : ((m468.reg[0] & 0x02) ? 0x03 : 0x07));
...
static void SetPRG_FME7(uint16 A, uint16 V) {
	uint16 prgMask = (m468.reg[0] & 0x02) ? 0x0F : 0x1F;
	uint16 prgBase = Mapper_GetPRGBase();
	setprg8(A, (prgBase & ~prgMask) | (V & prgMask));
}

static void SetCHR_FME7(uint16 A, uint16 V) {
	setchr1(A, V & 0xFF);
}
...
static void SetPRG_FxROM(uint16 A, uint16 V) {
	uint16 prgMask = (m468.reg[0] & 0x02) ? 0x07 : 0x0F;
	uint16 prgBase = Mapper_GetPRGBase();
	setprg16(A, (prgBase & ~prgMask) | (V & prgMask));
}

static void SetCHR_FxROM(uint16 A, uint16 V) {
	setchr4(A, V & 0xFF);
}
...
static void Sync_GNROM(void) {
	uint16 prgMask = (m468.reg[0] & 0x08) ? 0x01 : 0x03;
	uint16 prgBase = Mapper_GetPRGBase();
	uint8 value = latch.data;
...
static void SetPRG_SxROM(uint16 A, uint16 V) {
	uint16 prgMask = ((m468.reg[0] & 0x02) ? ((m468.reg[0] & 0x08) ? 0x03 : 0x07) : 0x0F);
	uint16 prgBase = Mapper_GetPRGBase();
	setprg16(A, (((prgBase >> 1) | (m468.reg[0] & 0x06)) & ~prgMask) | (V & prgMask));
}

static void SetCHR_SxROM(uint16 A, uint16 V) {
	setchr4(A, V & 0xFF);
}
...
static void SetPRG_TxROM(uint16 A, uint16 V) {
	uint16 prgMask = (m468.reg[0] & 0x08) ? ((m468.reg[0] & 0x04) ? (m468.reg[0] & 0x02 ? ((m468.reg[2] & 0x02) ? 0x07 : 0x0F) : 0x1F) : 0x3F) : 0x7F;
	uint16 prgBase = Mapper_GetPRGBase();
...
static void SetCHR_TxROM(uint16 A, uint16 V) {
	uint16 chrMask = m468.reg[0] & 0x10 ? 0xFF : 0x7F;
	setchr1(A, V & chrMask);
}
...
static void Sync_UxROM(void) {
	uint16 prgMask = (m468.reg[0] & 0x02) ? 0x07 : (((iNESCart.submapper == 1) && (~m468.reg[0] & 0x04)) ? 0x1F : 0x0F);
	uint16 prgBase = Mapper_GetPRGBase();
...
static void SetPRG_VRC1(uint16 A, uint16 V) {
	uint16 prgMask = m468.reg[0] & 0x08 ? (m468.reg[0] & 0x04 ? (m468.reg[0] & 0x02 ? 0x0F : 0x1F) : 0x3F) : 0x7F;
	uint16 prgBase = Mapper_GetPRGBase();
	setprg8(A, (prgBase & ~prgMask) | (V & prgMask));
}
